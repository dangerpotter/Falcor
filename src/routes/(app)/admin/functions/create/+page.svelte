<script>
	...
	const saveHandler = async (data) => {
		...
		if (compareVersion(manifest?.required_Falcor_version ?? '0.0.0', WEBUI_VERSION)) {
			console.log('Version is lower than required');
			toast.error(
				$i18n.t(
					'Falcor version (v{{Falcor_VERSION}}) is lower than required version (v{{REQUIRED_VERSION}})',
					{
						Falcor_VERSION: WEBUI_VERSION,
	...
	onMount(() => {
		window.addEventListener('message', async (event) => {
			if (
				!['https://strategicedinc.sharepoint.com/:u:/r/sites/CUCourseDev-OpsTeam/SitePages/Falcor.aspx?csf=1&web=1&e=uAImqs', 'https://www.Falcor.com', 'http://localhost:9999'].includes(
					event.origin
				)
			)
				return;
