...
from Falcor.config import CACHE_DIR
from Falcor.constants import ERROR_MESSAGES
from Falcor.env import ENV, SRC_LOG_LEVELS, ENABLE_FORWARD_USER_INFO_HEADERS

from Falcor.utils.auth import get_admin_user, get_verified_user
from Falcor.utils.images.comfyui import (
    ...
    try:
        if request.app.state.config.IMAGE_GENERATION_ENGINE == "openai":
            ...
            if ENABLE_FORWARD_USER_INFO_HEADERS:
                headers["X-Falcor-User-Name"] = user.name
                headers["X-Falcor-User-Id"] = user.id
                headers["X-Falcor-User-Email"] = user.email
                headers["X-Falcor-User-Role"] = user.role
